...
bool GUI_DrawFrame(SDL_Window* window, ViewerOptions& viewerOpts, MeshSimplificationOptions& options)
{
	ImGui::GetIO().MouseDrawCursor = false;
	ImGui_ImplSdl_NewFrame(window);
	SDL_ShowCursor(1);//options->showOptions ? 1 : 0);

	{
		ImGui::Begin("Options");
		
		char buf[1024];
		strcpy(buf, viewerOpts.modelPath.c_str());
		if (ImGui::InputText(".obj Path", buf, 1023))
		{
			viewerOpts.modelPath = buf;
		}
		...
		{
			ImGui::SliderFloat("Mesh Scale", &viewerOpts.meshScale, 0.f, 1000.f, "%.3f", 1.2f);
...
int main(int argc, char** argv)
{
	...
	std::string prevModelPath = viewerOpts.modelPath;

	MeshBuffer* meshBuffer = LoadMeshFromFile(viewerOpts.modelPath);
	Mesh mesh = CreateGLMesh(meshBuffer, viewerOpts.meshScale, options);
	...
	while (!quit)
	{
		...
		guiHasFocus = GUI_DrawFrame(window, viewerOpts, options);
		...
		if (viewerOpts.refreshModel)
		{
			...
			if (prevModelPath != viewerOpts.modelPath)
			{
				delete meshBuffer;
				meshBuffer = LoadMeshFromFile(viewerOpts.modelPath);
			}
